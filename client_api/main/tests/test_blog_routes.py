...
def test_blog_routes():
    ...
    with app.app_context():
        with app.test_client(use_cookies=True) as test_client:
            ...
            assert test_client.get("/tech-articles/categories/front-end"), "could not retrived Blog front end page"
            assert test_client.get("/tech-articles/categories/back-end"), "Could not retrieve Blog back end page"
            assert test_client.get("/tech-articles/categories/api"), "Could not retrieve blog API page"
            assert test_client.get("/tech-articles/categories/404"), "Could not retrieve blog 404 page"
